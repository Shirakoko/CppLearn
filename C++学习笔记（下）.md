...
`stack`是C++标准模板库中的一个**容器适配器**，提供了**先进后出(FILO, First In Last Out)**的数据结构功能
...
queue是C++标准模板库中的一个容器适配器，提供了先进先出(FIFO, First In First Out)的数据结构功能
...
queue<int> q;
if (q.empty()) {
    std::cout << "Queue is empty"; // 输出Queue is empty
}
